...
# Loading packages
In this part of the code file I simply load the packages we're going to use today. The `check.packages()` function is a custom one. It checks whether packages from the list provided below in the `pkg.list` vector are installed. If they are, it simply loads them; if they are not, it first installs them and then loads them.
...
# The list of packages that should be checked.
pkg.list <- c("tidyverse","scales","texreg","broom",
             "kableExtra","lavaan","psych","semPlot",
             "semTools","haven")
check.packages(pkg.list)
```
...
```{r fa-read-data}
anesDF <- read_dta("../02-data/03-ANES-pilot-1987.dta")
...
tempDF <- na.omit(anesDF[ ,c("a38a","a38b","a38c","a38d","a38e",
...
```{r pm-read-data}
jobsDF <- read_dta("../02-data/05-Jobs-NoMiss-Cont.dta")
str(jobsDF)
```
...
fit.ex <- sem(model.ex, data = jobsDF)
summary(fit.ex)
```
